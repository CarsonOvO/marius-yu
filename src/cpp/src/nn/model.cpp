...
torch::Tensor Model::forward_nc(at::optional<torch::Tensor> node_embeddings,
    ...
    // TEMP HACK: Inject dependency
    if (node_embeddings.has_value() && node_embeddings.value().defined()) {
        encoded_nodes = encoded_nodes + node_embeddings.value().sum() * 0;
    }

    // SPDLOG_INFO("encoded_nodes grad_fn != nullptr: {}", encoded_nodes.grad_fn() != nullptr);

    torch::Tensor y_pred = std::dynamic_pointer_cast<NodeDecoder>(decoder_)->forward(encoded_nodes);

    // SPDLOG_INFO("y_pred grad_fn != nullptr: {}", y_pred.grad_fn() != nullptr);
...
void Model::train_batch(std::shared_ptr<Batch> batch, bool call_step) {
    if (call_step) {
        clear_grad();
    }

    if (batch->node_embeddings_.defined()) {
        batch->node_embeddings_.set_requires_grad(true);
    }
    ...
    if (learning_task_ == LearningTask::LINK_PREDICTION) {
        ...
        if (batch->node_embeddings_.defined()) {
            try {
                auto grads = torch::autograd::grad({y_pred.sum()}, {batch->node_embeddings_}, {}, true, true);
                //SPDLOG_INFO("Grad w.r.t node_embeddings_ is defined: {}", grads[0].defined());
            } catch (const std::exception &e) {
                SPDLOG_WARN("grad() computation failed: {}", e.what());
            }
        }
    ...
    // SPDLOG_INFO("grad defined: {}", batch->node_embeddings_.grad().defined());
