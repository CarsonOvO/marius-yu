    ...
    for (int i = 0; i < layers_.size(); i++) {
        ...
        for (int j = 0; j < layers_[i].size(); j++) {
            if (instance_of<Layer, EmbeddingLayer>(layers_[i][j])) {
                auto narrowed = embeddings.value().narrow(0, 0, output_size).clone().detach().set_requires_grad(true);
                max_outputs[j] = std::dynamic_pointer_cast<EmbeddingLayer>(layers_[i][j])->forward(narrowed);
