...
void DataLoader::loadCPUParameters(shared_ptr<Batch> batch) {
    if (graph_storage_->storage_ptrs_.node_embeddings != nullptr) {
        if (graph_storage_->storage_ptrs_.node_embeddings->device_ != torch::kCUDA) {
            auto emb = graph_storage_->getNodeEmbeddings(batch->unique_node_indices_)
                           .detach()
                           .clone();                // 💡 确保是 leaf
            emb.set_requires_grad(true);             // 💡 启用梯度
            batch->node_embeddings_ = emb;
...
void DataLoader::loadGPUParameters(shared_ptr<Batch> batch) {
    if (graph_storage_->storage_ptrs_.node_embeddings != nullptr) {
        if (graph_storage_->storage_ptrs_.node_embeddings->device_ == torch::kCUDA) {
            auto emb = graph_storage_->getNodeEmbeddings(batch->unique_node_indices_)
                           .detach()
                           .clone();                  // 💡 断开计算图并复制，确保是 leaf tensor
            emb.set_requires_grad(true);             // 💡 启用梯度追踪
            batch->node_embeddings_ = emb;  
                     // 💡 赋值到 batch 中
            if (train_) {
                batch->node_embeddings_state_ = graph_storage_->getNodeEmbeddingState(batch->unique_node_indices_);
                SPDLOG_INFO("node_embeddings shape: [{}], element size: {}, total bytes: {}",
                    fmt::join(batch->node_embeddings_.sizes().vec(), ", "),
                    batch->node_embeddings_.element_size(),
                    batch->node_embeddings_.nbytes());
            }
    ...
    if (graph_storage_->storage_ptrs_.node_features != nullptr) {
        if (graph_storage_->storage_ptrs_.node_features->device_ == torch::kCUDA) {
            ...
            SPDLOG_INFO("node_features shape: [{}], element size: {}, total bytes: {}",
                fmt::join(batch->node_embeddings_.sizes().vec(), ", "),
                batch->node_features_.element_size(),
                batch->node_features_.nbytes());
