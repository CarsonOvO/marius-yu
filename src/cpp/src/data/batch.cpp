...
void Batch::accumulateGradients(float learning_rate) {
    if (node_embeddings_.defined() && node_embeddings_.requires_grad()) {
        auto grad = node_embeddings_.grad();
        if (grad.defined()) {
            node_gradients_ = grad;
            node_state_update_ = node_gradients_.pow(2);

            if (!node_embeddings_state_.defined()) {
                node_embeddings_state_ = torch::zeros_like(node_gradients_);
            }

            node_embeddings_state_.add_(node_state_update_);
            node_gradients_ = -learning_rate * (node_gradients_ / (node_embeddings_state_.sqrt().add(1e-10)));
        } else {
            SPDLOG_WARN("Gradient for node_embeddings_ is undefined; skipping update.");
        }
    }
