...
void GraphModelStorage::load() {
    ...
    if (train_) {
        ...
        try {
            std::string emb_file_path = "datasets/ogbn_arxiv/embeddings.bin";
            int64_t num_nodes = 169343;
            int64_t embedding_dim = 128;
    
            auto emb_storage = std::make_shared<InMemory>(
                emb_file_path,
                num_nodes,
                embedding_dim,
                torch::kFloat,
            ...
            emb_storage->load();
            storage_ptrs_.node_embeddings = emb_storage;
            torch::Tensor all_embeddings = emb_storage->range(0, num_nodes).to(torch::kCUDA);
            gpu_node_embeddings_ = all_embeddings;
            embeddings_loaded_to_gpu_ = true;
        
            SPDLOG_INFO("Embeddings loaded from raw binary file. Shape: [{} x {}]",
                        all_embeddings.size(0), all_embeddings.size(1));
        } catch (const std::exception &e) {
            SPDLOG_ERROR("Failed to load manual embedding file: {}", e.what());
        }
    
        _load(storage_ptrs_.node_optimizer_state);
        _load(storage_ptrs_.node_features);
        _load(storage_ptrs_.node_labels);
        storage_ptrs_.node_labels->load(); 
    
        if (!features_loaded_to_gpu_) {
            SPDLOG_INFO("Loading all node features into GPU memory via load()...");
            torch::Tensor indices = torch::arange(num_nodes_, torch::kLong);
            torch::Tensor cpu_feats = storage_ptrs_.node_features->indexRead(indices);
            gpu_node_features_ = cpu_feats.to(torch::kCUDA);
            features_loaded_to_gpu_ = true;
            SPDLOG_INFO("All features moved to GPU. Shape: [{}]", fmt::join(gpu_node_features_.sizes(), ", "));
        }
    } else {
        if (storage_ptrs_.node_features != nullptr) {
...
torch::Tensor GraphModelStorage::getNodeEmbeddings(Indices indices) {
    if (gpu_node_embeddings_.defined()) {
        return gpu_node_embeddings_
                   .index_select(0, indices.to(torch::kCUDA))
                   .clone()
                   .detach()
...
torch::Tensor GraphModelStorage::getNodeFeatures(Indices indices) {
    if (gpu_node_features_.defined()) {
        // Confirm full cache
        if (!gpu_features_verified_) {
            bool full_gpu_cache = (gpu_node_features_.size(0) == num_nodes_);
            if (full_gpu_cache) {
                SPDLOG_INFO(">> [GPU Feature Cache] All node features are cached in GPU.");
                gpu_features_verified_ = true;
            } else {
                SPDLOG_WARN(">> [GPU Feature Cache] gpu_node_features_ is defined but does not cover all nodes!");
            }
        }
        torch::Tensor cuda_indices = indices.to(torch::kCUDA);
        auto gpu_node_feats = gpu_node_features_.to(cuda_indices.device());
        torch::Tensor selected = gpu_node_features_.index_select(0, cuda_indices);
        TORCH_CHECK(selected.defined(), "gpu_node_features_.index_select returned an undefined tensor!");
        return selected;
    }

    if (!train_ &&
        instance_of<Storage, PartitionBufferStorage>(storage_ptrs_.node_features) &&
        full_graph_evaluation_) {

        if (in_memory_features_ != nullptr) {
            torch::Tensor selected = in_memory_features_->indexRead(indices);
            TORCH_CHECK(selected.defined(), "getNodeFeatures returned an undefined tensor from in_memory_features_!");
            return selected;
        } else {
            TORCH_CHECK(false, "in_memory_features_ is nullptr in getNodeFeatures()");
        }
    } else {
        if (storage_ptrs_.node_features != nullptr) {
            torch::Tensor selected = storage_ptrs_.node_features->indexRead(indices);
            TORCH_CHECK(selected.defined(), "getNodeFeatures returned an undefined tensor from storage_ptrs_.node_features!");
            return selected;
        } else {
            TORCH_CHECK(false, "storage_ptrs_.node_features is nullptr in getNodeFeatures()");
        }
    }

    // Redundant fallback
    TORCH_CHECK(false, "getNodeFeatures hit an unreachable path.");
    return torch::Tensor();
}
...
void GraphModelStorage::updatePutNodeEmbeddings(Indices indices, torch::Tensor embeddings) {
    if (gpu_node_embeddings_.defined()) {
        gpu_node_embeddings_.index_copy_(0, indices.to(torch::kCUDA), embeddings.to(torch::kCUDA));
    } else {
...
void GraphModelStorage::updateAddNodeEmbeddings(Indices indices, torch::Tensor values) {
    if (gpu_node_embeddings_.defined()) {
        gpu_node_embeddings_.index_add_(0, indices.to(torch::kCUDA), values.to(torch::kCUDA));
    } else {
