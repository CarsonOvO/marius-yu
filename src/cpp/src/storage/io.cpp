...
std::tuple<shared_ptr<Storage>, shared_ptr<Storage>> initializeNodeEmbeddings(shared_ptr<Model> model, shared_ptr<StorageConfig> storage_config,
    ...
    if (reinitialize) {
        SPDLOG_INFO("Reinitializing node embeddings.");
        ...
        while (offset < num_nodes) {
            curr_num_nodes = std::min(num_nodes - offset, static_cast<int64_t>(MAX_NODE_EMBEDDING_INIT_SIZE));
        ...
        SPDLOG_INFO("Node embeddings and optimizer state reinitialized.");
    } 
    else if (storage_config->embeddings->type == StorageBackend::PARTITION_BUFFER) {
        auto emb_options = std::dynamic_pointer_cast<PartitionBufferOptions>(storage_config->embeddings->options);
        if (emb_options != nullptr && emb_options->initialize_all && !fileExists(node_embedding_filename)) {
            SPDLOG_INFO("No embedding file found. Creating node embeddings with initialize_all=true.");
            ...
            SPDLOG_INFO("Node embedding fallback file creation complete.");
        }
    }
