...
def save_embedding(filename, num_nodes, embedding_dim, dtype=torch.float32, init="glorot_uniform"):
    if init == "glorot_uniform":
        limit = torch.sqrt(torch.tensor(6.0) / (num_nodes + embedding_dim))
        embeddings = torch.empty((num_nodes, embedding_dim), dtype=dtype)
        torch.nn.init.uniform_(embeddings, -limit.item(), limit.item())
    else:
        raise NotImplementedError("Only glorot_uniform is supported.")

    embeddings.numpy().astype("float32").tofile(filename)
    print(f"[TorchPartitioner] Saved raw binary embeddings: {filename} ({num_nodes} x {embedding_dim})")
...
class TorchPartitioner(Partitioner):
    def __init__(self, partitioned_evaluation, model_dir, embedding_dim, save_embeddings=True):
        super().__init__()
    ...
    def partition_edges(
        ...
        if self.save_embeddings:
            ...
            embedding_path = os.path.join(self.model_dir, "embeddings.bin")
            if not os.path.exists(embedding_path):
                save_embedding(embedding_path, num_nodes, self.embedding_dim)
            else:
                print(f"[TorchPartitioner] embeddings.bin already exists at {embedding_path}, skipping generation.")
